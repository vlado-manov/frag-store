const products = [
  {
    name: "Chanel No. 5",
    description:
      "Chanel No. 5 is a floral aldehyde fragrance composed of rose, jasmine, and sandalwood, creating a timeless and sophisticated scent.",
    brand: "Chanel",
    brand_slug: "chanel",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 75.0,
        discountPrice: 60.0,
        countInStock: 12,
        images: ["https://example.com/chanel50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 120.0,
        discountPrice: 95.0,
        countInStock: 5,
        images: ["https://example.com/chanel100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Chanel Gabrielle",
    description:
      "Gabrielle by Chanel is a radiant and luminous fragrance with a blend of jasmine, orange blossom, and tuberose.",
    brand: "Chanel",
    brand_slug: "chanel",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 90.0,
        discountPrice: 75.0,
        countInStock: 0,
        images: ["https://example.com/gabrielle50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 150.0,
        discountPrice: 130.0,
        countInStock: 2,
        images: ["https://example.com/gabrielle100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Chanel Bleu de Chanel",
    description:
      "Bleu de Chanel is a bold and fresh fragrance for male with notes of citrus, sandalwood, and cedar.",
    brand: "Chanel",
    brand_slug: "chanel",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 95.0,
        discountPrice: 80.0,
        countInStock: 1,
        images: ["https://example.com/bleu50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 155.0,
        discountPrice: 155.0,
        countInStock: 2,
        images: ["https://example.com/bleu50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 200,
        price: 205.0,
        discountPrice: 195.0,
        countInStock: 3,
        images: ["https://example.com/bleu50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Chanel Allure Homme Sport",
    description:
      "Allure Homme Sport is an energizing fragrance for male with notes of mandarin, cedar, and white musk.",
    brand: "Chanel",
    brand_slug: "chanel",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 110.0,
        discountPrice: 90.0,
        countInStock: 0,
        images: ["https://example.com/allure100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Chanel No. 19",
    description:
      "Chanel No. 19 is a classic green floral fragrance with notes of iris, rose, and galbanum, perfect for sophisticated tastes.",
    brand: "Chanel",
    brand_slug: "chanel",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 78.0,
        discountPrice: 65.0,
        countInStock: 4,
        images: ["https://example.com/no1950ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 120.0,
        discountPrice: 100.0,
        countInStock: 0,
        images: ["https://example.com/no19100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Dior Sauvage",
    description:
      "Dior Sauvage is a woody aromatic fragrance for male with notes of Calabrian bergamot, Sichuan pepper, and ambroxan.",
    brand: "Dior",
    brand_slug: "dior",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 85.0,
        discountPrice: 70.0,
        countInStock: 10,
        images: ["https://example.com/sauvage60ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 120,
        price: 150.0,
        discountPrice: 130.0,
        countInStock: 3,
        images: ["https://example.com/sauvage120ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Dior Miss Dior",
    description:
      "Miss Dior is a floral fragrance with notes of rose, peony, and lily of the valley, blended with a touch of patchouli for elegance.",
    brand: "Dior",
    brand_slug: "dior",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 95.0,
        discountPrice: 80.0,
        countInStock: 5,
        images: ["https://example.com/missdior50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 160.0,
        discountPrice: 140.0,
        countInStock: 0,
        images: ["https://example.com/missdior100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Dior J'adore",
    description:
      "J'adore is a radiant floral bouquet featuring notes of ylang-ylang, Damascus rose, and jasmine sambac.",
    brand: "Dior",
    brand_slug: "dior",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 105.0,
        discountPrice: 90.0,
        countInStock: 8,
        images: ["https://example.com/jadore50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 175.0,
        discountPrice: 150.0,
        countInStock: 0,
        images: ["https://example.com/jadore100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Dior Homme",
    description:
      "Dior Homme is a sophisticated and sensual fragrance for male with notes of iris, amber, and leather.",
    brand: "Dior",
    brand_slug: "dior",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 75.0,
        discountPrice: 65.0,
        countInStock: 0,
        images: ["https://example.com/homme50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 135.0,
        discountPrice: 120.0,
        countInStock: 4,
        images: ["https://example.com/homme100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Dior Fahrenheit",
    description:
      "Dior Fahrenheit is a bold and masculine fragrance with notes of leather, violet, and nutmeg.",
    brand: "Dior",
    brand_slug: "dior",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 80.0,
        discountPrice: 70.0,
        countInStock: 7,
        images: ["https://example.com/fahrenheit50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 140.0,
        discountPrice: 120.0,
        countInStock: 0,
        images: ["https://example.com/fahrenheit100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Dior Addict",
    description:
      "Dior Addict is a sensual oriental fragrance with notes of mandarin leaf, orange blossom, and bourbon vanilla.",
    brand: "Dior",
    brand_slug: "dior",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 100.0,
        discountPrice: 85.0,
        countInStock: 6,
        images: ["https://example.com/addict50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 170.0,
        discountPrice: 150.0,
        countInStock: 2,
        images: ["https://example.com/addict100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Dior Poison Girl",
    description:
      "Poison Girl is a seductive fragrance with a blend of bitter orange, tonka bean, and vanilla.",
    brand: "Dior",
    brand_slug: "dior",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 95.0,
        discountPrice: 75.0,
        countInStock: 0,
        images: ["https://example.com/poison50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 150.0,
        discountPrice: 130.0,
        countInStock: 0,
        images: ["https://example.com/poison100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },

  {
    name: "Tom Ford Black Orchid",
    description:
      "Tom Ford Black Orchid is a luxurious fragrance featuring dark chocolate, vanilla, and floral notes, perfect for evening wear.",
    brand: "Tom Ford",
    brand_slug: "tom-ford",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 110.0,
        discountPrice: 95.0,
        countInStock: 15,
        images: ["https://example.com/blackorchid50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 160.0,
        discountPrice: 140.0,
        countInStock: 8,
        images: ["https://example.com/blackorchid100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Creed Aventus",
    description:
      "Creed Aventus is a woody chypre fragrance with notes of pineapple, birch, and musk. A symbol of success and strength.",
    brand: "Creed",
    brand_slug: "creed",
    category: "niche",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 350.0,
        discountPrice: 320.0,
        countInStock: 5,
        images: ["https://example.com/aventus75ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 120,
        price: 450.0,
        discountPrice: 400.0,
        countInStock: 0,
        images: ["https://example.com/aventus120ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Yves Saint Laurent Libre",
    description:
      "Yves Saint Laurent Libre is an elegant fragrance blending lavender, orange blossom, and vanilla for a fresh, floral scent.",
    brand: "Yves Saint Laurent",
    brand_slug: "yves-saint-laurent",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 95.0,
        discountPrice: 80.0,
        countInStock: 10,
        images: ["https://example.com/libre50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 75,
        price: 135.0,
        discountPrice: 115.0,
        countInStock: 7,
        images: ["https://example.com/libre75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Gucci Guilty",
    description:
      "Gucci Guilty is a seductive fragrance with floral and citrus notes, ideal for those who enjoy bold, passionate scents.",
    brand: "Gucci",
    brand_slug: "gucci",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 85.0,
        discountPrice: 70.0,
        countInStock: 0,
        images: ["https://example.com/guilty75ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 110.0,
        discountPrice: 95.0,
        countInStock: 20,
        images: ["https://example.com/guilty100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Acqua di Gio",
    description:
      "Acqua di Gio is a fresh aquatic fragrance with notes of jasmine, lime, and rosemary, a classic scent for male.",
    brand: "Giorgio Armani",
    brand_slug: "giorgio-armani",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 100.0,
        discountPrice: 80.0,
        countInStock: 10,
        images: ["https://example.com/adiogio100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Jean Paul Gaultier Le Male",
    description:
      "Jean Paul Gaultier Le Male is a spicy, fresh fragrance with mint, lavender, and vanilla, offering a bold and warm scent.",
    brand: "Jean Paul Gaultier",
    brand_slug: "jean-paul-gaultier",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 125,
        price: 85.0,
        discountPrice: 75.0,
        countInStock: 5,
        images: ["https://example.com/lemale125ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Tom Ford Oud Wood",
    description:
      "Tom Ford Oud Wood is a warm, woody fragrance with notes of oud, vanilla, and amber, perfect for colder weather.",
    brand: "Tom Ford",
    brand_slug: "tom-ford",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 150.0,
        discountPrice: 130.0,
        countInStock: 8,
        images: ["https://example.com/oudwood50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 200.0,
        discountPrice: 180.0,
        countInStock: 15,
        images: ["https://example.com/oudwood100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Chloé Eau de Parfum",
    description:
      "Chloé Eau de Parfum is a delicate, feminine fragrance with notes of rose, peony, and lychee, perfect for everyday wear.",
    brand: "Chloé",
    brand_slug: "chloe",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 100.0,
        discountPrice: 85.0,
        countInStock: 12,
        images: ["https://example.com/chloe75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Viktor & Rolf Flowerbomb",
    description:
      "Viktor & Rolf Flowerbomb is a floral fragrance with notes of jasmine, orange blossom, and patchouli, creating a rich and bold scent.",
    brand: "Viktor & Rolf",
    brand_slug: "viktor-rolf",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 120.0,
        discountPrice: 100.0,
        countInStock: 20,
        images: ["https://example.com/flowerbomb50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 180.0,
        discountPrice: 160.0,
        countInStock: 5,
        images: ["https://example.com/flowerbomb100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Calvin Klein Eternity",
    description:
      "Calvin Klein Eternity is a fresh, floral fragrance with lily, jasmine, and violet notes.",
    brand: "Calvin Klein",
    brand_slug: "calvin-klein",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 65.0,
        discountPrice: 55.0,
        countInStock: 110,
        images: ["https://example.com/eternity50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Creed Green Irish Tweed",
    description:
      "Creed Green Irish Tweed is a fresh, green fragrance with a blend of lemon, verbena, and ambergris.",
    brand: "Creed",
    brand_slug: "creed",
    category: "niche",
    gender: "male",
    rating: 0,
    numReviews: 950,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 380.0,
        discountPrice: 350.0,
        countInStock: 60,
        images: ["https://example.com/greenirish100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Maison Francis Kurkdjian Baccarat Rouge 540",
    description:
      "Baccarat Rouge 540 is a luxurious fragrance with amber, saffron, and cedarwood notes, known for its bold and elegant scent.",
    brand: "Maison Francis Kurkdjian",
    brand_slug: "maison-francis-kurkdjian",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 875,
    reviews: [],
    variants: [
      {
        size: 70,
        price: 300.0,
        discountPrice: 280.0,
        countInStock: 50,
        images: ["https://example.com/baccarat54070ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Armani Code",
    description:
      "Armani Code is a sophisticated and seductive fragrance for male with notes of tonka bean, leather, and cedarwood.",
    brand: "Giorgio Armani",
    brand_slug: "giorgio-armani",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 85.0,
        discountPrice: 70.0,
        countInStock: 8,
        images: ["https://example.com/armanicode50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 120.0,
        discountPrice: 100.0,
        countInStock: 5,
        images: ["https://example.com/armanicode100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Si",
    description:
      "Si is a feminine fragrance with a blend of blackcurrant nectar, modern chypre, and musky blond wood.",
    brand: "Giorgio Armani",
    brand_slug: "giorgio-armani",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 90.0,
        discountPrice: 75.0,
        countInStock: 12,
        images: ["https://example.com/si50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 150.0,
        discountPrice: 130.0,
        countInStock: 0,
        images: ["https://example.com/si100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Acqua di Gioia",
    description:
      "Acqua di Gioia is a fresh, citrusy fragrance for woman, featuring notes of mint leaves, lemon, and water jasmine.",
    brand: "Giorgio Armani",
    brand_slug: "giorgio-armani",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 30,
        price: 65.0,
        discountPrice: 50.0,
        countInStock: 3,
        images: ["https://example.com/adiogioia30ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 50,
        price: 90.0,
        discountPrice: 75.0,
        countInStock: 0,
        images: ["https://example.com/adiogioia50ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Emporio Armani Stronger With You",
    description:
      "Stronger With You is a warm, spicy fragrance for male, with notes of cardamom, pink pepper, and vanilla.",
    brand: "Giorgio Armani",
    brand_slug: "giorgio-armani",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 85.0,
        discountPrice: 70.0,
        countInStock: 9,
        images: ["https://example.com/strongerwithyou60ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 120.0,
        discountPrice: 105.0,
        countInStock: 0,
        images: ["https://example.com/strongerwithyou100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Emporio Armani In Love With You",
    description:
      "In Love With You is a fruity floral fragrance for woman, featuring notes of cherry, raspberry, and vanilla.",
    brand: "Giorgio Armani",
    brand_slug: "giorgio-armani",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 90.0,
        discountPrice: 80.0,
        countInStock: 7,
        images: ["https://example.com/inlovewithyou50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Armani Prive Rouge Malachite",
    description:
      "Rouge Malachite is an intense oriental floral fragrance with notes of tuberose, amber, and orange blossom.",
    brand: "Giorgio Armani",
    brand_slug: "giorgio-armani",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 300.0,
        discountPrice: 280.0,
        countInStock: 2,
        images: ["https://example.com/rougemalachite100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Amouage Jubilation XXV",
    description:
      "A luxurious oriental fragrance with notes of frankincense, myrrh, and oud, creating a timeless scent.",
    brand: "Amouage",
    brand_slug: "amouage",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 350.0,
        discountPrice: 320.0,
        countInStock: 5,
        images: ["https://example.com/jubilationxxv100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Rasasi La Yuqawam",
    description:
      "A warm and spicy blend of leather, raspberry, and oud, offering a sophisticated and powerful scent.",
    brand: "Rasasi",
    brand_slug: "rasasi",
    category: "arabic",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 200.0,
        discountPrice: 180.0,
        countInStock: 3,
        images: ["https://example.com/layuqawam75ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Swiss Arabian Shaghaf Oud Aswad",
    description:
      "An intense and rich oud fragrance with notes of saffron, rose, and amber for a bold statemant.",
    brand: "Swiss Arabian",
    brand_slug: "swiss-arabian",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 150.0,
        discountPrice: 140.0,
        countInStock: 10,
        images: ["https://example.com/shaghafoudaswad75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Ajmal Amber Wood",
    description:
      "A captivating blend of amber, cedarwood, and patchouli, delivering a woody and oriental scent.",
    brand: "Ajmal",
    brand_slug: "ajmal",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 220.0,
        discountPrice: 200.0,
        countInStock: 7,
        images: ["https://example.com/amberwood100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Lattafa Oud Mood",
    description:
      "A sweet and woody oud fragrance with hints of vanilla, musk, and resin for a warm embrace.",
    brand: "Lattafa",
    brand_slug: "lattafa",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 120.0,
        discountPrice: 110.0,
        countInStock: 15,
        images: ["https://example.com/oudmood100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Al Haramain Amber Oud Gold Edition",
    description:
      "A luxurious blend of amber, oud, and citrus, creating an elegant and uplifting aroma.",
    brand: "Al Haramain",
    brand_slug: "al-haramain",
    category: "arabic",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 180.0,
        discountPrice: 170.0,
        countInStock: 8,
        images: ["https://example.com/amberoudgold60ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Khaltat Night Musk",
    description:
      "A rich and sensual fragrance with notes of white musk, jasmine, and sweet woods for a soothing aura.",
    brand: "Khaltat",
    brand_slug: "khaltat",
    category: "arabic",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 90.0,
        discountPrice: 80.0,
        countInStock: 12,
        images: ["https://example.com/nightmusk50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Arabian Oud Kalemat",
    description:
      "A warm and sweet oud fragrance with notes of honey, musk, and vanilla for a cozy and inviting scent.",
    brand: "Arabian Oud",
    brand_slug: "arabian-oud",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 250.0,
        discountPrice: 230.0,
        countInStock: 6,
        images: ["https://example.com/kalemat100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Nabeel Black Musk",
    description:
      "A dark and mysterious fragrance with notes of musk, patchouli, and smoky woods, perfect for evening wear.",
    brand: "Nabeel",
    brand_slug: "nabeel",
    category: "arabic",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 140.0,
        discountPrice: 130.0,
        countInStock: 10,
        images: ["https://example.com/blackmusk100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Afnan Supremacy in Oud",
    description:
      "A bold and elegant oud fragrance with hints of spices, florals, and amber for a refined scent.",
    brand: "Afnan",
    brand_slug: "afnan",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 190.0,
        discountPrice: 175.0,
        countInStock: 8,
        images: ["https://example.com/supremacyinoud100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Armani Prive Vert Malachite",
    description:
      "Vert Malachite is a soft floral fragrance with green accents, featuring notes of lily, jasmine, and vanilla.",
    brand: "Giorgio Armani",
    brand_slug: "giorgio-armani",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 300.0,
        discountPrice: 280.0,
        countInStock: 4,
        images: ["https://example.com/vertmalachite100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },

  {
    name: "Paco Rabanne 1 Million",
    description:
      "Paco Rabanne 1 Million is a spicy and woody fragrance with notes of cinnamon, leather, and amber, designed for a bold and confident man.",
    brand: "Paco Rabanne",
    brand_slug: "paco-rabanne",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 85.0,
        discountPrice: 70.0,
        countInStock: 15,
        images: ["https://example.com/1million50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 100,
        price: 120.0,
        discountPrice: 105.0,
        countInStock: 0,
        images: ["https://example.com/1million100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Versace Bright Crystal",
    description:
      "Versace Bright Crystal is a fresh, floral fragrance featuring pomegranate, peony, and lotus flower.",
    brand: "Versace",
    brand_slug: "versace",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 85.0,
        discountPrice: 75.0,
        countInStock: 18,
        images: ["https://example.com/brightcrystal50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Hugo Boss Bottled",
    description:
      "Hugo Boss Bottled is a classic fragrance for male, with apple, cinnamon, and sandalwood notes.",
    brand: "Hugo Boss",
    brand_slug: "hugo-boss",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 95.0,
        discountPrice: 80.0,
        countInStock: 20,
        images: ["https://example.com/bottled100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Chanel Chance Eau Tendre",
    description:
      "Chanel Chance Eau Tendre is a delicate fragrance with notes of grapefruit, jasmine, and musk, offering a light and refreshing scent.",
    brand: "Chanel",
    brand_slug: "chanel",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 110.0,
        discountPrice: 90.0,
        countInStock: 12,
        images: ["https://example.com/chanceeautendre50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Dolce & Gabbana Light Blue",
    description:
      "Dolce & Gabbana Light Blue is a vibrant fragrance with notes of Sicilian lemon, apple, and cedarwood.",
    brand: "Dolce & Gabbana",
    brand_slug: "dolce-gabbana",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 85.0,
        discountPrice: 75.0,
        countInStock: 8,
        images: ["https://example.com/lightblue100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Givenchy Gentlemale Only",
    description:
      "Givenchy Gentlemale Only is an elegant fragrance with spicy and woody notes of pepper, birch leaf, and leather.",
    brand: "Givenchy",
    brand_slug: "givenchy",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 95.0,
        discountPrice: 85.0,
        countInStock: 15,
        images: ["https://example.com/gentlemaleonly100ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 200,
        price: 160.0,
        discountPrice: 140.0,
        countInStock: 5,
        images: ["https://example.com/gentlemaleonly200ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Acqua di Parma Colonia",
    description:
      "Acqua di Parma Colonia is a classic citrus fragrance with notes of Sicilian citrus, lavender, and rosemary, ideal for a refreshing feel.",
    brand: "Acqua di Parma",
    brand_slug: "acqua-di-parma",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 150.0,
        discountPrice: 130.0,
        countInStock: 18,
        images: ["https://example.com/colonia100ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 150,
        price: 180.0,
        discountPrice: 160.0,
        countInStock: 0,
        images: ["https://example.com/colonia150ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Guerlain Shalimar",
    description:
      "Guerlain Shalimar is an oriental fragrance with rich notes of vanilla, iris, and incense, creating a luxurious and warm aura.",
    brand: "Guerlain",
    brand_slug: "guerlain",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 80.0,
        discountPrice: 65.0,
        countInStock: 12,
        images: ["https://example.com/shalimar50ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 90,
        price: 120.0,
        discountPrice: 100.0,
        countInStock: 5,
        images: ["https://example.com/shalimar90ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Hermès Eau d'Orange Verte",
    description:
      "Hermès Eau d'Orange Verte is a fresh, zesty fragrance with notes of orange, mint, and moss, perfect for casual wear.",
    brand: "Hermès",
    brand_slug: "hermes",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 150.0,
        discountPrice: 140.0,
        countInStock: 10,
        images: ["https://example.com/eaudorange100ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 200,
        price: 200.0,
        discountPrice: 180.0,
        countInStock: 0,
        images: ["https://example.com/eaudorange200ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Ajmal Mukhallat Dahn Al Oudh Moattaq",
    description:
      "A pure and traditional oud fragrance with a blend of rose, amber, and saffron for a luxurious experience.",
    brand: "Ajmal",
    brand_slug: "ajmal",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 320.0,
        discountPrice: 300.0,
        countInStock: 4,
        images: ["https://example.com/mukhallatoudh60ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Prada La Femme",
    description:
      "A sensual fragrance with notes of frangipani, ylang-ylang, and tuberose.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 105.0,
        discountPrice: null,
        countInStock: 7,
        images: ["https://example.com/lafemme50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 145.0,
        discountPrice: null,
        countInStock: 5,
        images: ["https://example.com/lafemme100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Cedrat Boise",
    description:
      "Свеж и енергичен аромат с нотки на цитруси, черен касис и сандалово дърво.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 120.0,
        discountPrice: null,
        countInStock: 5,
        images: ["https://example.com/cedratboise60ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 120,
        price: 160.0,
        discountPrice: 150.0,
        countInStock: 3,
        images: ["https://example.com/cedratboise120ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Roses Vanille",
    description:
      "Сладък и романтичен аромат с нотки на роза, ванилия и мускус.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 110.0,
        discountPrice: 100.0,
        countInStock: 6,
        images: ["https://example.com/rosesvanille60ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 120,
        price: 150.0,
        discountPrice: null,
        countInStock: 4,
        images: ["https://example.com/rosesvanille120ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Red Tobacco",
    description:
      "Интензивен аромат с топли нотки на канела, тютюн и дървесина.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 120,
        price: 170.0,
        discountPrice: 160.0,
        countInStock: 5,
        images: ["https://example.com/redtobacco120ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Black Gold",
    description:
      "Луксозен аромат с нотки на лавданум, сандалово дърво и мускус.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 200.0,
        discountPrice: null,
        countInStock: 3,
        images: ["https://example.com/blackgold100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Sicily",
    description: "Свеж и плодов аромат с нотки на цитруси, праскова и жасмин.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 125.0,
        discountPrice: 115.0,
        countInStock: 7,
        images: ["https://example.com/sicily60ml.jpg"],
        video: "",
        onSale: true,
      },
      {
        size: 120,
        price: 180.0,
        discountPrice: null,
        countInStock: 4,
        images: ["https://example.com/sicily120ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Aoud Lemon Mint",
    description:
      "Екзотичен аромат с комбинация от лимон, мента и агарово дърво.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 120,
        price: 150.0,
        discountPrice: 140.0,
        countInStock: 6,
        images: ["https://example.com/aoudlemonmint120ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Wild Cherry",
    description: "Забавен и игрив аромат с нотки на череша, ванилия и мускус.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 115.0,
        discountPrice: 105.0,
        countInStock: 8,
        images: ["https://example.com/wildcherry60ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Hindu Kush",
    description:
      "Интензивен и мистичен аромат с нотки на смола, тамян и мускус.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 120,
        price: 190.0,
        discountPrice: null,
        countInStock: 5,
        images: ["https://example.com/hindukush120ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Velvet Vanilla",
    description:
      "Сладък и кремообразен аромат с нотки на ванилия, тубероза и круша.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 120.0,
        discountPrice: null,
        countInStock: 6,
        images: ["https://example.com/velvetvanilla60ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 120,
        price: 160.0,
        discountPrice: 150.0,
        countInStock: 3,
        images: ["https://example.com/velvetvanilla120ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Musky Garden",
    description: "Свеж и флорален аромат с нотки на цитруси, роза и мускус.",
    brand: "Mancera",
    brand_slug: "mancera",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 120,
        price: 180.0,
        discountPrice: null,
        countInStock: 4,
        images: ["https://example.com/muskygarden120ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Prada Amber Pour Homme",
    description:
      "A timeless fragrance with notes of amber, vanilla, and tonka bean.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 90.0,
        discountPrice: null,
        countInStock: 6,
        images: ["https://example.com/amberpourhomme50ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Prada Les Infusions de Mimosa",
    description:
      "A delicate and refreshing fragrance featuring notes of mimosa, rose, and mandarin.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 120.0,
        discountPrice: 110.0,
        countInStock: 4,
        images: ["https://example.com/lesinfusionsmimosa50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Prada Luna Rossa Black",
    description:
      "A bold and mysterious fragrance combining notes of amber, patchouli, and musk.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 100.0,
        discountPrice: null,
        countInStock: 5,
        images: ["https://example.com/lunarossablack50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 140.0,
        discountPrice: 130.0,
        countInStock: 3,
        images: ["https://example.com/lunarossablack100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Prada Infusion d'Amande",
    description:
      "A luxurious fragrance with almond, heliotrope, and tonka bean.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 135.0,
        discountPrice: null,
        countInStock: 4,
        images: ["https://example.com/infusiondamande100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Prada Candy Night",
    description:
      "A captivating fragrance with notes of orange, cocoa, and caramel.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 115.0,
        discountPrice: null,
        countInStock: 7,
        images: ["https://example.com/candynight50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 80,
        price: 150.0,
        discountPrice: 140.0,
        countInStock: 5,
        images: ["https://example.com/candynight80ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Prada Paradoxe",
    description:
      "A floral ambery fragrance that reinvents freshness with a blend of neroli, amber, and musk.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 30,
        price: 85.0,
        discountPrice: null,
        countInStock: 10,
        images: ["https://example.com/paradoxe30ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 50,
        price: 115.0,
        discountPrice: null,
        countInStock: 8,
        images: ["https://example.com/paradoxe50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 90,
        price: 160.0,
        discountPrice: null,
        countInStock: 5,
        images: ["https://example.com/paradoxe90ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Prada Luna Rossa Ocean",
    description:
      "A fresh and invigorating fragrance combining notes of bergamot, iris, and vetiver.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 120.0,
        discountPrice: null,
        countInStock: 7,
        images: ["https://example.com/lunarossa_ocean50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 150.0,
        discountPrice: null,
        countInStock: 6,
        images: ["https://example.com/lunarossa_ocean100ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 150,
        price: 190.0,
        discountPrice: null,
        countInStock: 4,
        images: ["https://example.com/lunarossa_ocean150ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Prada Candy",
    description:
      "A playful and addictive fragrance with notes of caramel, musk, and benzoin.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 30,
        price: 75.0,
        discountPrice: null,
        countInStock: 9,
        images: ["https://example.com/candy30ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 50,
        price: 105.0,
        discountPrice: null,
        countInStock: 7,
        images: ["https://example.com/candy50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 80,
        price: 135.0,
        discountPrice: null,
        countInStock: 5,
        images: ["https://example.com/candy80ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Prada Infusion d'Iris",
    description:
      "A fresh and elegant fragrance with notes of iris, mandarin, and cedarwood.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 110.0,
        discountPrice: null,
        countInStock: 6,
        images: ["https://example.com/infusiondiris50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 150.0,
        discountPrice: null,
        countInStock: 4,
        images: ["https://example.com/infusiondiris100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Prada L'Homme",
    description:
      "A sophisticated fragrance blending notes of neroli, geranium, and patchouli.",
    brand: "Prada",
    brand_slug: "prada",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 95.0,
        discountPrice: null,
        countInStock: 8,
        images: ["https://example.com/lhomme50ml.jpg"],
        video: "",
        onSale: false,
      },
      {
        size: 100,
        price: 125.0,
        discountPrice: null,
        countInStock: 5,
        images: ["https://example.com/lhomme100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Lattafa Raghba",
    description:
      "A sweet and smoky fragrance with notes of vanilla, oud, and incense, perfect for any occasion.",
    brand: "Lattafa",
    brand_slug: "lattafa",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 95.0,
        discountPrice: 85.0,
        countInStock: 15,
        images: ["https://example.com/raghba100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Swiss Arabian Kashkha",
    description:
      "A vibrant and spicy scent with cinnamon, cardamom, and oud for a distinctive and bold aroma.",
    brand: "Swiss Arabian",
    brand_slug: "swiss-arabian",
    category: "arabic",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 130.0,
        discountPrice: 120.0,
        countInStock: 8,
        images: ["https://example.com/kashkha50ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Rasasi Qasamat Morhaf",
    description:
      "A sophisticated oriental fragrance with a unique blend of floral, amber, and oud notes.",
    brand: "Rasasi",
    brand_slug: "rasasi",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 65,
        price: 160.0,
        discountPrice: 150.0,
        countInStock: 5,
        images: ["https://example.com/qasamatmorhaf65ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Arabian Oud Majestic",
    description:
      "A luxurious oud and amber scent with hints of spices and musk, offering a royal experience.",
    brand: "Arabian Oud",
    brand_slug: "arabian-oud",
    category: "arabic",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 300.0,
        discountPrice: 280.0,
        countInStock: 6,
        images: ["https://example.com/majestic100ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Nabeel Spirit of Dubai Oud",
    description:
      "A masterpiece of oud with layers of floral, woody, and spicy notes for an unforgettable scent.",
    brand: "Nabeel",
    brand_slug: "nabeel",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 90,
        price: 400.0,
        discountPrice: 380.0,
        countInStock: 3,
        images: ["https://example.com/spiritofdubaioud90ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Lattafa Ana Abiyedh Rouge",
    description:
      "A bright and fruity fragrance with notes of saffron, jasmine, and amber, perfect for daytime wear.",
    brand: "Lattafa",
    brand_slug: "lattafa",
    category: "arabic",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 60,
        price: 85.0,
        discountPrice: 75.0,
        countInStock: 12,
        images: ["https://example.com/anaabiyedhrouge60ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Ajmal Evoke Gold",
    description:
      "An aromatic fragrance with spicy and floral notes, complemaleted by a base of sandalwood and musk.",
    brand: "Ajmal",
    brand_slug: "ajmal",
    category: "arabic",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 110.0,
        discountPrice: 100.0,
        countInStock: 10,
        images: ["https://example.com/evokegold75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Khaltat Al Arabia Royal Blend",
    description:
      "A royal blend of oud, amber, and musk, with floral and fruity undertones for a balanced and elegant scent.",
    brand: "Khaltat Al Arabia",
    brand_slug: "khaltat-al-arabia",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 180.0,
        discountPrice: 165.0,
        countInStock: 7,
        images: ["https://example.com/royalblend100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Swiss Arabian Oud Maknoon",
    description:
      "A deep and mysterious oud fragrance with rich woody and smoky notes for an enigmatic presence.",
    brand: "Swiss Arabian",
    brand_slug: "swiss-arabian",
    category: "arabic",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 120.0,
        discountPrice: 110.0,
        countInStock: 9,
        images: ["https://example.com/oudmaknoon50ml.jpg"],
        video: "",
        onSale: false,
      },
    ],
  },
  {
    name: "Narciso Rodriguez For Her",
    description:
      "Narciso Rodriguez For Her is a soft, sensual fragrance with musky, floral notes, including rose, peach, and amber.",
    brand: "Narciso Rodriguez",
    brand_slug: "narciso-rodriguez",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 95.0,
        discountPrice: 80.0,
        countInStock: 18,
        images: ["https://example.com/forher75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "L'Occitane Terre de Lumière",
    description:
      "Terre de Lumière is a warm, radiant fragrance with notes of honey, lavender, and almond, offering a cozy and inviting feel.",
    brand: "L'Occitane",
    brand_slug: "loccitane",
    category: "niche",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 90,
        price: 85.0,
        discountPrice: 70.0,
        countInStock: 12,
        images: ["https://example.com/terre90ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Bvlgari Man in Black",
    description:
      "Bvlgari Man in Black is a bold and intense fragrance with notes of rum, leather, and spices, perfect for an adventurous spirit.",
    brand: "Bvlgari",
    brand_slug: "bvlgari",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 115.0,
        discountPrice: 100.0,
        countInStock: 20,
        images: ["https://example.com/maninblack100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },

  {
    name: "Viktor & Rolf Bonbon",
    description:
      "Bonbon by Viktor & Rolf is a sweet fragrance with notes of caramel, orange, and jasmine, perfect for those who love gourmand scents.",
    brand: "Viktor & Rolf",
    brand_slug: "viktor-rolf",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 120.0,
        discountPrice: 100.0,
        countInStock: 15,
        images: ["https://example.com/bonbon50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Chanel Coco Mademoiselle",
    description:
      "Chanel Coco Mademoiselle is a vibrant, elegant fragrance with notes of orange, jasmine, and patchouli.",
    brand: "Chanel",
    brand_slug: "chanel",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 140.0,
        discountPrice: 120.0,
        countInStock: 0,
        images: ["https://example.com/cocomademoiselle100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Bond No. 9 New York Oud",
    description:
      "Bond No. 9 New York Oud is a luxurious and complex fragrance with oud, rose, and amber, designed to evoke the spirit of New York.",
    brand: "Bond No. 9",
    brand_slug: "bond-no-9",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 450.0,
        discountPrice: 420.0,
        countInStock: 30,
        images: ["https://example.com/nyoud100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Le Labo Rose 31",
    description:
      "Rose 31 by Le Labo is a unique, woody floral fragrance with notes of rose, cumin, and cedarwood.",
    brand: "Le Labo",
    brand_slug: "le-labo",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 175.0,
        discountPrice: 150.0,
        countInStock: 4,
        images: ["https://example.com/rose31.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Cartier La Panthère",
    description:
      "La Panthère by Cartier is a feline-inspired fragrance with notes of gardenia, musk, and chypre.",
    brand: "Cartier",
    brand_slug: "cartier",
    category: "niche",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 130.0,
        discountPrice: 110.0,
        countInStock: 25,
        images: ["https://example.com/panthere75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Montale Intense Cafe",
    description:
      "Montale Intense Cafe is a warm, gourmand fragrance with notes of rose, vanilla, and coffee, offering a cozy and inviting vibe.",
    brand: "Montale",
    brand_slug: "montale",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 185.0,
        discountPrice: 160.0,
        countInStock: 13,
        images: ["https://example.com/intensecafe100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Burberry Her",
    description:
      "Burberry Her is a fruity-floral fragrance with notes of blackberry, jasmine, and amber.",
    brand: "Burberry",
    brand_slug: "burberry",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 100.0,
        discountPrice: 85.0,
        countInStock: 9,
        images: ["https://example.com/her50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Calvin Klein Euphoria",
    description:
      "Calvin Klein Euphoria is a rich, exotic fragrance with notes of pomegranate, orchid, and mahogany.",
    brand: "Calvin Klein",
    brand_slug: "calvin-klein",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 80.0,
        discountPrice: 70.0,
        countInStock: 11,
        images: ["https://example.com/euphoria100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },

  {
    name: "Issey Miyake L'Eau d'Issey",
    description:
      "Issey Miyake L'Eau d'Issey is a fresh, aquatic fragrance with lotus, rose, and woodsy notes.",
    brand: "Issey Miyake",
    brand_slug: "issey-miyake",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 85.0,
        discountPrice: 75.0,
        countInStock: 12,
        images: ["https://example.com/eaudissey75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Jean Patou Joy",
    description:
      "Jean Patou Joy is a luxurious floral fragrance with notes of rose, jasmine, and ylang-ylang.",
    brand: "Jean Patou",
    brand_slug: "jean-patou",
    category: "niche",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 250.0,
        discountPrice: 220.0,
        countInStock: 15,
        images: ["https://example.com/joy50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Byredo Gypsy Water",
    description:
      "Byredo Gypsy Water is a woody fragrance with notes of bergamot, juniper berries, and sandalwood.",
    brand: "Byredo",
    brand_slug: "byredo",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 200.0,
        discountPrice: 180.0,
        countInStock: 3,
        images: ["https://example.com/gypsywater100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Maison Margiela Replica Jazz Club",
    description:
      "Replica Jazz Club is a warm, smoky fragrance with rum, tobacco, and vanilla notes.",
    brand: "Maison Margiela",
    brand_slug: "maison-margiela",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 135.0,
        discountPrice: 115.0,
        countInStock: 4,
        images: ["https://example.com/jazzclub100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Le Labo Santal 33",
    description:
      "Le Labo Santal 33 is a woody fragrance with sandalwood, cedarwood, and leather notes.",
    brand: "Le Labo",
    brand_slug: "le-labo",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 200.0,
        discountPrice: 180.0,
        countInStock: 17,
        images: ["https://example.com/santal33.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Tom Ford Neroli Portofino",
    description:
      "Tom Ford Neroli Portofino is a citrusy fragrance with neroli, bergamot, and orange blossom notes.",
    brand: "Tom Ford",
    brand_slug: "tom-ford",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 160.0,
        discountPrice: 140.0,
        countInStock: 6,
        images: ["https://example.com/neroliosportofino50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Aerin Lauder Rose de Grasse",
    description:
      "Aerin Lauder Rose de Grasse is a luxurious floral fragrance with rose, jasmine, and bergamot notes.",
    brand: "Aerin Lauder",
    brand_slug: "aerin-lauder",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 180.0,
        discountPrice: 160.0,
        countInStock: 5,
        images: ["https://example.com/rosegdegrasse50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Hermès Terre d'Hermès",
    description:
      "Hermès Terre d'Hermès is a woody, earthy fragrance with orange, pepper, and vetiver notes.",
    brand: "Hermès",
    brand_slug: "hermes",
    category: "designer",
    gender: "male",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 150.0,
        discountPrice: 130.0,
        countInStock: 8,
        images: ["https://example.com/terredhermes75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Jo Malone London English Pear & Freesia",
    description:
      "Jo Malone London English Pear & Freesia is a fresh, floral fragrance with pear, freesia, and patchouli notes.",
    brand: "Jo Malone London",
    brand_slug: "jo-malone-london",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 100,
        price: 130.0,
        discountPrice: 120.0,
        countInStock: 7,
        images: ["https://example.com/englishpear100ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Diptyque Philosykos",
    description:
      "Diptyque Philosykos is a fresh, green fragrance with fig, coconut, and cedarwood notes.",
    brand: "Diptyque",
    brand_slug: "diptyque",
    category: "niche",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 75,
        price: 135.0,
        discountPrice: 120.0,
        countInStock: 15,
        images: ["https://example.com/philosykos75ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Bvlgari Omnia Crystalline",
    description:
      "Bvlgari Omnia Crystalline is a fresh, floral fragrance with notes of bamboo, pear, and lotus flower.",
    brand: "Bvlgari",
    brand_slug: "bvlgari",
    category: "designer",
    gender: "female",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 65,
        price: 75.0,
        discountPrice: 65.0,
        countInStock: 10,
        images: ["https://example.com/omnia65ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
  {
    name: "Tom Ford Tobacco Vanille",
    description:
      "Tom Ford Tobacco Vanille is a warm, spicy fragrance with tobacco, vanilla, and dried fruit notes.",
    brand: "Tom Ford",
    brand_slug: "tom-ford",
    category: "designer",
    gender: "unisex",
    rating: 0,
    numReviews: 0,
    reviews: [],
    variants: [
      {
        size: 50,
        price: 210.0,
        discountPrice: 190.0,
        countInStock: 6,
        images: ["https://example.com/tobaccovanille50ml.jpg"],
        video: "",
        onSale: true,
      },
    ],
  },
];

export default products;
